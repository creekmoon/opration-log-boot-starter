...
public class ReflectUtil {
    ...
    public static <T> Constructor<T> getConstructor(Class<T> clazz, Class<?>... parameterTypes) {
        ...
        for (Constructor<?> constructor : constructors) {
            ...
            if (cn.hutool.core.util.ClassUtil.isAllAssignableFrom(pts, parameterTypes)) {
                // 构造可访问
    ...
    public static Field[] getFields(Class<?> beanClass, Filter<Field> fieldFilter) throws SecurityException {
        return cn.hutool.core.util.ArrayUtil.filter(getFields(beanClass), fieldFilter);
    }
    ...
    public static Field[] getFieldsDirectly(Class<?> beanClass, boolean withSuperClassFields) throws SecurityException {
        ...
        while (searchType != null) {
            ...
            if (null == allFields) {
                ...
                allFields = cn.hutool.core.util.ArrayUtil.append(allFields, declaredFields);
            }
    ...
    public static Object getFieldValue(Object obj, String fieldName) throws UtilException {
        if (null == obj || cn.hutool.core.util.StrUtil.isBlank(fieldName)) {
            return null;
        }
     ...
     * 若字段类型是原始类型而传入的值是 null，则会将字段设置为对应原始类型的默认值（见 {@link cn.hutool.core.util.ClassUtil#getDefaultValue(Class)}）
     ...
     * 设置字段值<br>
     * 若值类型与字段类型不一致，则会尝试通过 {@link Convert} 进行转换<br>
     * 若字段类型是原始类型而传入的值是 null，则会将字段设置为对应原始类型的默认值（见 {@link cn.hutool.core.util.ClassUtil#getDefaultValue(Class)}）<br>
    ...
    public static void setFieldValue(Object obj, Field field, Object value) throws UtilException {
        ...
        if (null != value) {
            ...
            value = cn.hutool.core.util.ClassUtil.getDefaultValue(fieldType);
        }
    ...
    public static Set<String> getPublicMethodNames(Class<?> clazz) {
        ...
        if (cn.hutool.core.util.ArrayUtil.isNotEmpty(methodArray)) {
    ...
    public static Method getMethodOfObj(Object obj, String methodName, Object... args) throws SecurityException {
        if (null == obj || cn.hutool.core.util.StrUtil.isBlank(methodName)) {
            return null;
        }
        return getMethod(obj.getClass(), methodName, cn.hutool.core.util.ClassUtil.getClasses(args));
    }
    ...
    public static Method getMethod(Class<?> clazz, boolean ignoreCase, String methodName, Class<?>... paramTypes) throws SecurityException {
        ...
        if (cn.hutool.core.util.ArrayUtil.isNotEmpty(methods)) {
            for (Method method : methods) {
                if (cn.hutool.core.util.StrUtil.equals(methodName, method.getName(), ignoreCase)
    ...
    public static Method[] getMethods(Class<?> clazz, Filter<Method> filter) throws SecurityException {
        ...
        return cn.hutool.core.util.ArrayUtil.filter(getMethods(clazz), filter);
    }
    ...
    public static <T> T newInstance(Class<T> clazz, Object... params) throws UtilException {
        ...
        final Class<?>[] paramTypes = cn.hutool.core.util.ClassUtil.getClasses(params);
    ...
    public static <T> T newInstanceIfPossible(Class<T> type) {
        ...
        // 原始类型
        if (type.isPrimitive()) {
            return (T) cn.hutool.core.util.ClassUtil.getPrimitiveDefaultValue(type);
        }
        ...
        for (Constructor<T> constructor : constructors) {
            ...
            setAccessible(constructor);
            try {
                return constructor.newInstance(cn.hutool.core.util.ClassUtil.getDefaultValues(parameterTypes));
        ...
        if (null != args) {
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                if (type.isPrimitive() && null == args[i]) {
                    ...
                    args[i] = cn.hutool.core.util.ClassUtil.getDefaultValue(type);
                }
            }
        }
        ...
        if (null != args) {
            for (int i = 0; i < actualArgs.length; i++) {
                if (i >= args.length || null == args[i]) {
                    // 越界或者空值
                    actualArgs[i] = cn.hutool.core.util.ClassUtil.getDefaultValue(parameterTypes[i]);
