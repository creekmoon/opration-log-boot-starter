...
- **CSV 导出**：支持导出实时统计、TopN排行、历史趋势为CSV格式（锚点：`HeatmapService#exportRealtimeStatsToCsv` / `HeatmapExportController`）
...
- `cn.creekmoon.operationLog.heatmap.HeatmapExportController` - CSV导出Controller
- `cn.creekmoon.operationLog.export.CsvExportService` - CSV导出服务
...
7. 通过 `HeatmapExportController` 暴露CSV导出接口

## 6. 外部依赖

- Redis（HyperLogLog 命令：PFADD、PFCOUNT、PFMERGE）
- Apache Commons CSV（CSV导出）
- Caffeine（本地降级缓存）
...
- **如何导出 CSV**：访问 `GET /operation-log/heatmap/export/realtime` 或 `/export/topn` 或 `/export/trend`
